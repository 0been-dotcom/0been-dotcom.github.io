...
설치 과정은 생각보다 매끄러웠지만, llama-server가 8080 포트를 제대로监听하고 있는지 확인하는 단계에서 잠시 막혔던 적이 있습니다. 포트 충돌이나 방화벽 설정을 꼼꼼히 체크하면 대부분 해결되므로, 초기 설정 시 네트워크 상태를 반드시 확인하는 게 좋습니다.

첫 실행 시 가장 놀랐던 점은 Carnice-V2-27B 모델이 48GB RAM 환경에서 얼마나 가볍고 빠르게 응답하느냐는 점입니다. Q4_K_M 양자화 덕분에 추론 속도가 매우 쾌적해서, 로컬에서这么大的 모델을 돌린다는 게 실제 체감상 믿기지 않을 정도였습니다.

초보자가 가장 헷갈리는 부분은 아마도 모델 파일의 경로나 llama-server와의 연결 설정일 것입니다. 에이전트와 백엔드 서버가 서로 통신할 때 사용하는 API 엔드포인트 주소를 정확히 일치시켜야 하는데, 이 부분이 설정 파일에서 조금씩 달라질 수 있어 주의가 필요합니다.

Mac mini M4 Pro 환경에서는 유니버설 앱이 아닌 경우 일부 라이브러리 호환성 문제가 발생할 수 있으니, Apple Silicon 전용 빌드를 사용하는 것이 중요합니다. 특히 메모리 관리가 매우 효율적이어서 48GB라는 큰 용량을 활용하면서도 시스템이 무거워지지 않는 점이 큰 장점으로 느껴졌습니다.

추천하는 첫 번째 실습 과제는 간단한 정보 검색이나 요약 작업을 에이전트에게 맡겨보는 것입니다. 복잡한 코드 작성보다는 일상적인 질문을 통해 응답 품질과 속도를 직접 체험해보는 것이, 시스템의 안정성을 파악하는 데 가장 도움이 될 것입니다.

...
설치 과정 자체는 공식 문서대로 따라 하면 비교적 매끄럽게 진행되었지만, 네트워크 설정이나 의존성 충돌로 인한 에러가 가끔 발생해 해결에 시간이 걸렸습니다. 특히 로컬 LLM 서버와의 연동 설정 부분에서 포트 충돌 여부를 먼저 체크하는 것이 중요했습니다.

처음 실행했을 때 가장 놀랐던 점은 Carnice-V2-27B 모델이 생각보다 빠른 응답 속도를 보여주었다는 점입니다. 48GB RAM의 여유로운 환경 덕분에 문맥 유지가 잘 되었고, 복잡한 지시에도 논리적으로 잘 따라오는 체감이 인상적이었습니다.

초보자가 가장 많이 헷갈리는 부분은 llama-server와의 통신 설정과 Hermes Agent의 프롬프트 구조입니다. 로컬 서버가 정상적으로 구동되고 있는지 확인하지 않고 Agent를 실행하면 연결 오류가 발생하므로, 이 부분을 반드시 먼저 점검해야 합니다.

Mac mini M4 Pro 환경에서는 Apple Silicon 최적화 덕분에 CPU/GPU 리소스 할당이 효율적이었습니다. 다만, 메모리 관리를 위해 불필요한 백그라운드 프로세스를 최소화했을 때 가장 안정적인 성능을 확인할 수 있었습니다.

첫 번째 실습 과제로는 간단한 문서 요약이나 이메일 초안 작성과 같은 텍스트 생성 작업을 추천합니다. 이를 통해 모델의 응답 품질과 지연 시간을 직접 체감해 보며, 자신에게 맞는 설정값을 찾는 데 도움이 될 것입니다.
